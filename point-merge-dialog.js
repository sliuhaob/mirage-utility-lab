import {mergeCandidates,pointGroupKey,samePointDefinition} from './lineup-groups.js?v=2';
...
export async function confirmPointMerges(data,candidates,previous,{notice=''}={}){
 ...
 return new Promise(resolve=>{
  ...
  for(const kind of ['target','origin']){
   ...
   const add=(value,label,description)=>{const row=el('label');row.className='merge-choice';const radio=el('input');radio.type='radio';radio.name=kind;radio.value=value;radio.required=true;const copy=el('span');copy.append(el('strong',label),el('small',description));row.append(radio,copy);field.append(row);};
