...
export async function chooseSpawn(config,team,current){
 ...
 try{
  if(!catalog){const response=await fetch('./assets/spawn-points.json?v=1',{signal:AbortSignal.any([controller.signal,AbortSignal.timeout(10000)])});if(!response.ok)throw Error('出生点读取失败');const data=await response.json();if(!data.maps?.[config.id]?.length)throw Error('出生点数据无效');catalog=data;}
